...
#### 设计目标
- 编译时处理，零运行时开销。
- 支持纯 CSS 和 Sass（扩展 Less）。
- 通过构建脚本 + 外部 `dart-sass` 二进制实现。

#### 实现方案

1. 宏 `defineCSS`：计算内容 hash，写入缓存目录，生成 `include_css!` 宏调用。
2. 构建脚本：扫描缓存目录，调用 `dart-sass` 编译。
3. `include_css!` 宏：读取编译后 CSS 文件，嵌入字符串字面量。
...
- [x] `defineCSS` Sass 模式（`@sass` 宏 + `dart-sass` CLI 集成）。
- [ ] WS 主动推送。
- [ ] 开发模式热重载。
- [ ] WS 断线重连与 session 恢复。
- [ ] 应用配置（theme、layout）。
