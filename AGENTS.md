...
- `@EmbedString("public/css/element-plus.css")` 按 `cjpm build` 时的**当前工作目录**解析路径，在 `examples/` 下编译会读取 `examples/public/css/element-plus.css`。因此 `build-css.sh` 必须同时输出到 root 和 `examples/public/css/element-plus.css`。
...
- `@EmbedString("public/css/element-plus.css")` 按 `cjpm build` 时的**当前工作目录**解析路径；在 `examples/` 下编译会读取 `examples/public/css/element-plus.css`。因此 `build-css.sh` 必须同步输出到 root 和 `examples/public/css/element-plus.css`。
...
- 根因：`@EmbedString("public/css/element-plus.css")` 按 `cjpm build` 时的**当前工作目录**解析路径。我们在 `examples/` 下执行 `cjpm build`，它读取的是 `examples/public/css/element-plus.css`（旧缓存，不含 dialog 样式），而不是 root 的 `public/css/element-plus.css`。
