...
| `--static` 编译 (examples) | ✅ |
| **GranularPatch 强类型系统**（枚举 + CombinedPatchMsg） | ✅ |
| **前端子树替换**（renderSubtree + replaceChild） | ✅ |
| Issue #5: nextHid() 每次 render 产生新 ID | ✅ diff 跳过 actions 比较 + refresh 追加 handler |
| **Input bind value 修复**（el.value 替代 setAttribute） | ✅ |
| **删除按钮修复**（恢复 sendPatch 中 state.refresh） | ✅ |
| InputsPage/TodoPage 统一闭包捕获 | ⬜ 低优先级 |
| HTTP 信号残留清理 | ⬜ 最低优先级 |
...
| #8 | `中` `优化` `服务端` | Component-level 脏标记跳过不变子树 diff |
