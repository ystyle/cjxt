...
**问题 1：嵌套宏 `setItem`/`getChildMessages` 可行**
- 参考 storm-cj（`@Model` + `@Id`/`@Index`）和 kux-cj（`@Crud` + `@BeforeCreate`）已验证，展开顺序为内层先展开，外层用 `getChildMessages(key)` 收集。
- 无需 `assertParentContext`，同宏包内即可工作。
...
**问题 4：`${}` 在 JS 中与 Cangjie `quote` 插值冲突**
- `$(text)` 嵌入的 JS 内容若含 `${}` 会被 `quote` 当作 Cangjie 插值解析。
- 需在 JS 中转义为 `\${}`。
